...
public class xmgAugmentedFace : xmgAugmentedFaceBase
{
    ...
    void Start()
    {
        ...
        if (!m_videoParameters.useNativeCapture)
		{
            ...
            {
                m_capturePlane = (xmgVideoCapturePlaneFace)gameObject.AddComponent(typeof(xmgVideoCapturePlaneFace));
        ...
        xmgAugmentedFaceBridge.xzimgSetCalibration(
            xmgToolsFace.ConvertFov(fovy_degree, arVideo) * 3.1415 / 180.0,             // we want fovx in rds
            m_videoParameters.GetProcessingWidth(),
            m_videoParameters.GetProcessingHeight());
    ...
    void Update ()
    {
...
#elif (!UNITY_EDITOR && UNITY_IOS)
            int res = xmgAugmentedFaceBridge.tmp_xzimgCamera_getImage(m_capturePlane.m_PixelsHandle.AddrOfPinnedObject());
#endif
        }
        ...
        {
            trackingParams.m_rotate_mode = (int)xmgToolsFace.GetDeviceCurrentOrientation((int)m_captureDeviceOrientation, m_videoParameters.UseFrontal) ;
        }

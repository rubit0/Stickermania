...
public class xmgAugmentedFramedImage : xmgAugmentedVisionBase
{
    ...
    public override void Start()
    {
        ...
        int success = xmgAugmentedVisionBridge.xzimgFramedImageInitialize(
    ...
    {
        base.OnDisable();
        xmgAugmentedVisionBridge.xzimgFramedImageRelease();
	}
    ...
    public override void Update()
    {
        ...
        // -- Detection
        int ret = xmgAugmentedVisionBridge.xzimgFramedImageDetect(
            ref m_image, m_visionParameters.FilterStrength, 0, 1, 1);

        // -- Rendering
        DisableObjects();
        int iNbrOfDetection = xmgAugmentedVisionBridge.xzimgFramedImageGetNumber();
        ...
        if (iNbrOfDetection > 0)
        {
            for (int i = 0; i < iNbrOfDetection; i++)
            {
                ...
                xmgAugmentedVisionBridge.xzimgFramedImageGetInfoForUnity(i, ref markerInfo);
    ...
    void LoadImages()
	{
		for (int i = 0; i < m_visionParameters.ObjectPivotLinks.Count; i++)
		{
			if (m_visionParameters.ObjectPivotLinks[i].Classifier)
			{
				...
				int success = xmgAugmentedVisionBridge.xzimgFramedImageAddTarget(

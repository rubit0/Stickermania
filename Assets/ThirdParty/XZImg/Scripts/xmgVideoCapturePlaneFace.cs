...
public class xmgVideoCapturePlaneFace : MonoBehaviour
{
    ...
    /// </summary>
    public WebCamTexture OpenVideoCapture(ref xmgVideoCaptureParametersFace videoParameters)
    {
        ...
        else
        {
            ...
            if (m_webcamTexture.width != m_webcamTexture.requestedWidth && m_webcamTexture.requestedWidth > 100 && m_webcamTexture.width > 100)
            {
                ...
                videoParameters.videoCaptureMode = xmgVideoCaptureParametersFace.getVideoCaptureMode(m_webcamTexture.width, m_webcamTexture.height);
            }
        }
        return m_webcamTexture;
    }

    public void CreateVideoCapturePlane(float screenScaleFactor, xmgVideoCaptureParametersFace videoParameters)
    {
        m_captureWidth = videoParameters.GetVideoCaptureWidth();
        m_captureHeight = videoParameters.GetVideoCaptureHeight();
        ...
        GetComponent<Renderer>().material = new Material(Shader.Find("Custom/VideoShaderFace"));
        ...
        // shader parameters
        GetComponent<Renderer>().material.SetInt("_Rotation", (int)xmgToolsFace.GetVideoOrientation(videoParameters.useNativeCapture, videoParameters.UseFrontal));
    ...
    static public float GetScaleX(xmgVideoCaptureParametersFace videoParameters)
    {
    ...
    static public float GetScaleY(xmgVideoCaptureParametersFace videoParameters)
    {
        int CaptureWidth = videoParameters.GetVideoCaptureWidth();
        int CaptureHeight = videoParameters.GetVideoCaptureHeight();
    ...
    public void SwitchCamera(ref xmgVideoCaptureParametersFace videoParameters)
    {
...
#if (!UNITY_EDITOR && UNITY_IOS)
        if (videoParameters.UseFrontal)         // frontal image is inverted
            GetComponent<Renderer>().material.SetInt(
                "_Mirror",
                (int)(videoParameters.MirrorVideo == true ? 0 : 1));
        GetComponent<Renderer>().material.SetInt(
            "_VerticalMirror",

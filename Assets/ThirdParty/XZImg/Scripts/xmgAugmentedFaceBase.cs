   ...
   public  void UpdateObjectPosition()
    ...
    public void PrepareCamera()
    {
        ...
        if (m_videoParameters.videoPlaneFittingMode == xmgVideoPlaneFittingMode.FitScreenHorizontally &&
            (xmgToolsFace.GetRenderOrientation() == xmgOrientationMode.LandscapeLeft || xmgToolsFace.GetRenderOrientation() == xmgOrientationMode.LandscapeRight))
        {
            float fovx = (float)xmgToolsFace.ConvertFov(m_videoParameters.CameraVerticalFOV, m_videoParameters.GetVideoAspectRatio());
            Camera.main.fieldOfView = (float)xmgToolsFace.ConvertFov(fovx, 1.0f / m_videoParameters.GetScreenAspectRatio());
        }
        ...
        if (m_videoParameters.videoPlaneFittingMode == xmgVideoPlaneFittingMode.FitScreenHorizontally &&
            (xmgToolsFace.GetRenderOrientation() == xmgOrientationMode.Portrait || xmgToolsFace.GetRenderOrientation() == xmgOrientationMode.PortraitUpsideDown))
        {
            Camera.main.fieldOfView = (float)xmgToolsFace.ConvertFov(m_videoParameters.CameraVerticalFOV, m_videoParameters.GetVideoAspectRatio());
        }

        if (m_videoParameters.videoPlaneFittingMode == xmgVideoPlaneFittingMode.FitScreenVertically &&
            (xmgToolsFace.GetRenderOrientation() == xmgOrientationMode.Portrait || xmgToolsFace.GetRenderOrientation() == xmgOrientationMode.PortraitUpsideDown))
        {
            Camera.main.fieldOfView = (float)xmgToolsFace.ConvertFov(m_videoParameters.CameraVerticalFOV, m_videoParameters.GetScreenAspectRatio());
        }
    ...
    public void SwitchCamera()
    {
        ...
        int front = 1 - videoParams.m_frontal;
        xmgAugmentedFaceBridge.tmp_xzimgCamera_delete();
        videoParams.m_frontal = front;
        m_videoParameters.MirrorVideo = front==0?false:true;
        m_videoParameters.UseFrontal = front==0?false:true;  
        m_capturePlane.SwitchCamera(ref m_videoParameters);
        xmgAugmentedFaceBridge.tmp_xzimgCamera_create(ref videoParams);
#endif

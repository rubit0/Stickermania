...
namespace LeoLuz.PlugAndPlayJoystick
{
    public class AnalogicKnob : ButtonBase
    {
        ...
        private void OnEnable()
        {
            if (wasDisabled)
            {
                StartCoroutine(WasDisabledCoroutine());
            }
        }
        ...
        private IEnumerator WasDisabledCoroutine()
        {
        ...
        void Update()
        {
            ...
            // Workaround when panel was disabled
            if (wasDisabled)
            {
                Input.SetAxisMobile(HorizontalAxis, 0f);
                Input.SetAxisMobile(VerticalAxis, 0f);
            }
            else
            {

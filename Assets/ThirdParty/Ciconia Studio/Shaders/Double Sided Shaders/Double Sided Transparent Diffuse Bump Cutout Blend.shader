Shader "Ciconia Studio/Double Sided/Transparent/Diffuse Bump Cutout Blend" {
    Properties {
        _Color ("Diffuse Color", Color) = (1,1,1,1)
        _MainTex ("Diffuse map (Cutout A)", 2D) = "white" {}
        _SecondaryTex ("Second Diffuse map (Cutout A)", 2D) = "white" {}
        _Fade ("Fade between diffuse textures", Range(0, 1)) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float4 secondTexture = tex2D(_SecondaryTex, TRANSFORM_TEX(i.uv0, _SecondaryTex));

                float4 _MainTex_var = lerp(mainTexture, secondTexture, _Fade);
